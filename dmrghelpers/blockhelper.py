...
def update_to_leftblock(lbkext: LeftBlockExtend, spphival, skipcheck=False):
    ...
    '''
    #对应phi值的alpha
    newstanum = spphival.shape[0]#len(phival)
    pnum_list = []
    #
    ...
    return lbkext.merge_to_block(spphival, pnum_list=pnum_list)


def update_to_rightblock(rbkext: RightBlockExtend, spphival, skipcheck=False):
    '''将一组phi带入21.6式，这个phi应该是通过求密度矩阵本正态得到的
    ...
    for beta in range(newstanum):
        ...
        pnum_list.append(pnum)
    return rbkext.merge_to_block(spphival, pnum_list=pnum_list)
